...
def get_bible_shape(books: List[Book], verses: List[Verse]) -> Dict[str, Dict[int, int]]:
    bible_shape: Dict[str, Dict[int, int]] = {}
    for book in books:
        bible_shape[book.name] = {}
        ...
        for chapter in range(1, number_of_chapters + 1):
            ...
            bible_shape[book.name][chapter] = len(verses_of_chapter)
    return bible_shape
